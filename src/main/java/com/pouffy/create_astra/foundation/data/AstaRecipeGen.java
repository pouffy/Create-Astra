...
public abstract class AstaRecipeGen extends CreateRecipeProvider {
	...
	public AstaRecipeGen(FabricDataGenerator generator) {
		super(generator);
	}
	...
	<T extends ProcessingRecipe<?>> GeneratedRecipe create(Supplier<ItemLike> singleIngredient,
		...
		return create(CreateAstra.ID, singleIngredient, transform);
	}
	...
	<T extends ProcessingRecipe<?>> GeneratedRecipe create(String name,
														   UnaryOperator<ProcessingRecipeBuilder<T>> transform) {
		return create(CreateAstra.asResource(name), transform);
	}
	...
	protected Supplier<ResourceLocation> idWithSuffix(Supplier<ItemLike> item, String suffix) {
		return () -> {
			ResourceLocation registryName = RegisteredObjects.getKeyOrThrow(item.get()
					.asItem());
			return CreateAstra.asResource(registryName.getPath() + suffix);
		};
	}

	@Override
	public String getName() {
		return "Create: Astra's Recipes: " + getRecipeType().getId()
				.getPath();
	}
}
